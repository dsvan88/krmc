class ImagesPad extends Prompt {
    checkboxs = [];
    values = [];
    constructor({ title = "Mafia Vote Numpad", text = "Choose images:", value = '', action = null, cancel = null, images = [] } = {}) {
        value = '';
        super({ title, text, value, action, cancel })
        this.modifyForImagesPad(block).modifyEventsNumpad();
        this.dialog.focus();
        this.images = images;
    }

    modifyForImagesPad(block = []) {
