<section class='section form'>
    <form action="/news/edit/promo" method="post" enctype="multipart/form-data" class="form__form">
        <h2 class="form__title"><?= $title ?></h2>
        <div><input type="hidden" name="type" value="<?= $newsData['type'] ?>"></div>
        <?/*<div>
            <label for="news-logo-input-file">
                Зображення
                <input type="file" name="logo" value="<?= $newsData['logo'] ?>" placeholder="Logo" class="form__input logo" id="news-logo-input-file">
            </label>
        </div>*/ ?>
