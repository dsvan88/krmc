...
class TodayCommand extends ChatCommand
{
    ...
    public static function execute(array $arguments = [], string &$message = '', string &$reaction = '', array &$replyMarkup = [])
    {
        ...
        $replyMarkup = [
            'inline_keyboard' => [
                [
                    ['text' => self::locale('I will too!'), 'callback_data' => static::replyButton(['c' => 'booking', 'w' => $weekData['id'], 'd' => $currentDayNum])],
                    ['text' => self::locale('I will too! I hope...'), 'callback_data' => static::replyButton(['c' => 'booking', 'w' => $weekData['id'], 'd' => $currentDayNum, 'p' => '?'])],
                ],
            ],
        ];
        return true;
