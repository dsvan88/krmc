...
class TelegramBotService
{
    ...
    {
        ChatAction::$arguments['currentDay'] = Day::current();

        $daySlug = mb_strtolower($daySlug, 'UTF-8');
        if (mb_strlen($daySlug, 'UTF-8') > 3) {
            $daySlug = mb_substr($daySlug, 0, 3);
        }
        if (in_array($daySlug, DayService::$techDaysArray['today'], true)) {
            ChatAction::$arguments['dayNum'] = ChatAction::$arguments['currentDay'];
            return;
        }
        if (in_array($daySlug, DayService::$techDaysArray['tomorrow'], true)) {
        ...
        foreach (DayService::$daysArray as $num => $daysNames) {
