...
class NickCommand extends ChatCommand
{
    ...
    public static function execute()
    {
        ...
        if (empty(trim($_username))) {
            ...
            TelegramChatsService::setPendingState('nick');
            ...
            $replyMarkup = [
                'inline_keyboard' => [
                    [
                        ['text' => '❌' . self::locale('Cancel'), 'callback_data' => ['c' => 'pending', 'p' => 'nick', 'ci' => TelegramBotService::getUserTelegramId()]],
                    ],
                ],
            ];
            return [
        ...
        if (empty($userExistsData['id'])) {
            ...
            TelegramChatsService::getAndSaveTgAvatar($userId, true);
        ...
        $userContacts = ContactService::formatUserContacts($userContacts);

        $message = self::locale(['string' => '{{ Tg_Command_Name_Already_Set_By_Other }}', 'vars' => [$username]]);

        $isChatExists = TelegramChatsService::isChatExists($userContacts['telegramid']);
        $isAvailable = AccountService::checkAvailable($userExistsData['id']);
